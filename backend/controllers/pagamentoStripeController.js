...
const webhookStripe = async (req, res) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (event.type === "checkout.session.expired") {
      ...
      if (assinaturaId) {
        console.log(`⚠️ Checkout expirado. Atualizando assinatura ${assinaturaId} para CANCELADA.`);
        ...
        console.log(`❌ Assinatura ${assinaturaId} marcada como CANCELADA (checkout expirado).`);
      } else {
        console.warn("Evento 'checkout.session.expired' recebido sem assinaturaId nos metadados.");
      }
    }
